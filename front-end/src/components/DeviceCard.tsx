      ...
      <CardContent className="p-0 text-white font-mono">
        ...
        <div className="p-6 border-b border-white/5 bg-gradient-to-b from-[#00e5bf]/5 to-transparent">
          <div className="flex justify-between items-start mb-6">
            <div className="flex items-center gap-4">
              <div className={cn("p-3 rounded-xl border-2", device.is_online ? "border-[#00e5bf] text-[#00e5bf]" : "border-gray-800 text-gray-800")}>
                <Monitor className="h-6 w-6" />
              </div>
              <div>
                <h3 className="text-sm font-black tracking-widest uppercase">{device.device_id}</h3>
                <span className="text-[10px] text-[#00e5bf] font-bold">
                  {device.is_online ? "TRACKING_LIVE" : "CONNECTION_INTERRUPTED"}
                </span>
              </div>
            </div>
            <div className="flex items-center gap-4">
              <div className="text-right">
                <p className="text-[9px] text-gray-500 uppercase font-bold">Total_Uptime</p>
                <p className="text-xl font-black">{formatTime(device.total_screen_time)}</p>
              </div>
              <button onClick={() => setIsExpanded(!isExpanded)} className="p-2 rounded-lg bg-white/5 hover:bg-white/10 border border-white/10">
          ...
          <div className="border border-[#00e5bf]/20 bg-black/60 p-4 rounded-xl flex gap-4">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-[9px] text-[#00e5bf] uppercase font-black">Current_Process</p>
              <h2 className="text-lg font-black truncate">{getActiveAppLabel(device.is_online, device.current_app)}</h2>
              <p className="text-xs text-gray-400 truncate">&gt; {device.website || "exec_system_idle"}</p>
            </div>
          </div>
        </div>
        ...
        {isExpanded && (
          <div className="grid grid-cols-1 lg:grid-cols-[250px_1fr]">
            {/* PROCESS STACK - FULL BACKGROUND VERSION */}
            <div className="p-5 border-r border-white/5 bg-black/40">
              <div className="flex items-center gap-2 mb-5">
                <Cpu size={14} className="text-[#bc00ff]" />
                <span className="text-[10px] font-black uppercase text-gray-400">Process_Stack</span>
              </div>
                  ...
                  return (
                    <button
                      ...
                      className={cn(
                        ...
                        isSelected 
                          ? "border-transparent text-[#080d10]" // Dark text on bright background
                          : "border-white/5 text-[#666] hover:border-white/20"
                      )}
                      style={isSelected ? { backgroundColor: color, boxShadow: `0 0 15px ${color}40` } : {}}
                    >
                      <span className="text-[11px] font-black truncate lowercase">{name}</span>
                      <span className="text-[10px] font-bold opacity-80 shrink-0">{formatTime(time)}</span>
                    </button>
                  );
                })}
              </div>
            ...
            {/* RIGHT PANEL - WEB HISTORY WITH PROGRESS BARS */}
              ...
              {isBrowser(selectedApp) && activeWebEntries.length > 0 ? (
                <>
                  <div className="flex items-center justify-between mb-6">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-[10px] uppercase font-black text-cyan-400">Network_Traffic_Logs // {selectedApp}</span>
                    </div>
                    <span className="text-[10px] text-[#00e5bf] font-mono tracking-widest">TOTAL_WEB_TIME: {formatTime(totalWebTime)}</span>
                  </div>
                      ...
                      return (
                        <div key={site} className="group relative border-b border-white/5 pb-3">
                          <div className="flex items-center justify-between gap-4 mb-2">
                            <div className="flex-1 min-w-0">
                              <span className="text-xs text-gray-300 truncate block group-hover:text-[#00e5bf] transition-colors" title={site}>
                                {cleanSiteName(site)}
                              </span>
                            </div>
                ...
                <div className="flex flex-col h-full items-center justify-center">
                  <div className="w-full flex items-center gap-2 mb-2">
                    ...
                    <span className="text-[10px] uppercase font-black text-gray-500">Data_Distribution_Model</span>
                  </div>
                  <AppUsagePie data={device.app_usage} totalTime={device.total_screen_time} selectedApp={selectedApp} />
                </div>
              )}
            </div>
          </div>
        ...
        <div className="px-6 py-4 border-t border-white/5 bg-black/60 flex justify-between">
          <span className="text-[9px] text-gray-600 uppercase font-black">Parental_Watch_Active</span>
          <span className="text-[9px] text-gray-700 font-mono">{new Date(device.last_updated).toLocaleString()}</span>
        </div>
      </CardContent>
    </Card>
  );
